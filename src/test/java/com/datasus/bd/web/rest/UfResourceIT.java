...
class UfResourceIT {
    ...
    @Test
    void getAllUfsAsStream() {
        // Initialize the database
        ufRepository.save(uf).block();

        List<Uf> ufList = webTestClient
            .get()
            .uri(ENTITY_API_URL)
            ...
            .returnResult(Uf.class)
            .getResponseBody()
            .filter(uf::equals)
            .collectList()
            .block(Duration.ofSeconds(5));

        assertThat(ufList).isNotNull();
        assertThat(ufList).hasSize(1);
        Uf testUf = ufList.get(0);
        assertThat(testUf.getCodigoIbge()).isEqualTo(DEFAULT_CODIGO_IBGE);
        assertThat(testUf.getEstado()).isEqualTo(DEFAULT_ESTADO);
        assertThat(testUf.getBandeira()).isEqualTo(DEFAULT_BANDEIRA);
        assertThat(testUf.getBandeiraContentType()).isEqualTo(DEFAULT_BANDEIRA_CONTENT_TYPE);
    ...
    void getAllUfs() {
        ...
        webTestClient
            ...
            .expectBody()
            .jsonPath("$.[*].id")
            .value(hasItem(uf.getId().intValue()))
            .jsonPath("$.[*].codigoIbge")
    ...
    void getUf() {
        ...
        webTestClient
            .get()
            .uri(ENTITY_API_URL_ID, uf.getId())
            .accept(MediaType.APPLICATION_JSON)
            .exchange()
            .expectStatus()
            .isOk()
    ...
    void putNewUf() throws Exception {
        ...
        // Update the uf
        Uf updatedUf = ufRepository.findById(uf.getId()).block();
        updatedUf
            .codigoIbge(UPDATED_CODIGO_IBGE)
            .estado(UPDATED_ESTADO)
            .bandeira(UPDATED_BANDEIRA)
        ...
        webTestClient
            .put()
            .uri(ENTITY_API_URL_ID, updatedUf.getId())
            .contentType(MediaType.APPLICATION_JSON)
    ...
    void putNonExistingUf() throws Exception {
        int databaseSizeBeforeUpdate = ufRepository.findAll().collectList().block().size();
        uf.setId(count.incrementAndGet());
