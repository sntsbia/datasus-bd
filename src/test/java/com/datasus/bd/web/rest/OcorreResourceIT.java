...
class OcorreResourceIT {
    ...
    @Test
    void getAllOcorresAsStream() {
        // Initialize the database
        ocorreRepository.save(ocorre).block();

        List<Ocorre> ocorreList = webTestClient
            .get()
            .uri(ENTITY_API_URL)
            ...
            .returnResult(Ocorre.class)
            .getResponseBody()
            .filter(ocorre::equals)
            .collectList()
            .block(Duration.ofSeconds(5));

        assertThat(ocorreList).isNotNull();
        assertThat(ocorreList).hasSize(1);
        Ocorre testOcorre = ocorreList.get(0);
    }
