...
class MoraResourceIT {
    ...
    @Test
    void getAllMorasAsStream() {
        // Initialize the database
        moraRepository.save(mora).block();

        List<Mora> moraList = webTestClient
            .get()
            .uri(ENTITY_API_URL)
            ...
            .returnResult(Mora.class)
            .getResponseBody()
            .filter(mora::equals)
            .collectList()
            .block(Duration.ofSeconds(5));

        assertThat(moraList).isNotNull();
        assertThat(moraList).hasSize(1);
        Mora testMora = moraList.get(0);
    }
