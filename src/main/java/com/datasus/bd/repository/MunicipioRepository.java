...
public interface MunicipioRepository extends ReactiveCrudRepository<Municipio, Long>, MunicipioRepositoryInternal {
    @Query("SELECT * FROM municipio entity WHERE entity.uf_id = :id")
    Flux<Municipio> findByUf(Long id);

    @Query("SELECT * FROM municipio entity WHERE entity.uf_id IS NULL")
    Flux<Municipio> findAllWhereUfIsNull();
