...
public class TomaResource {
     ...
     */
    @GetMapping("/tomas")
    public Mono<List<Toma>> getAllTomas() {
        log.debug("REST request to get all Tomas");
        return tomaRepository.findAll().collectList();
    }

    /**
     * {@code GET  /tomas} : get all the tomas as a stream.
     * @return the {@link Flux} of tomas.
     */
    @GetMapping(value = "/tomas", produces = MediaType.APPLICATION_NDJSON_VALUE)
    public Flux<Toma> getAllTomasAsStream() {
        log.debug("REST request to get all Tomas as a stream");
