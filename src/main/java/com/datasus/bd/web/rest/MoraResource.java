...
public class MoraResource {
     ...
     */
    @GetMapping("/moras")
    public Mono<List<Mora>> getAllMoras() {
        log.debug("REST request to get all Moras");
        return moraRepository.findAll().collectList();
    }

    /**
     * {@code GET  /moras} : get all the moras as a stream.
     * @return the {@link Flux} of moras.
     */
    @GetMapping(value = "/moras", produces = MediaType.APPLICATION_NDJSON_VALUE)
    public Flux<Mora> getAllMorasAsStream() {
        log.debug("REST request to get all Moras as a stream");
