...
public class OcorreResource {
     ...
     */
    @GetMapping("/ocorres")
    public Mono<List<Ocorre>> getAllOcorres() {
        log.debug("REST request to get all Ocorres");
        return ocorreRepository.findAll().collectList();
    }

    /**
     * {@code GET  /ocorres} : get all the ocorres as a stream.
     * @return the {@link Flux} of ocorres.
     */
    @GetMapping(value = "/ocorres", produces = MediaType.APPLICATION_NDJSON_VALUE)
    public Flux<Ocorre> getAllOcorresAsStream() {
        log.debug("REST request to get all Ocorres as a stream");
        return ocorreRepository.findAll();
    }
