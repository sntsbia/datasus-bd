...
describe('Municipio Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call Uf query and add missing value', () => {
      const municipio: IMunicipio = { idMunicipio: 456 };
      const fkIdUf: IUf = { idUf: 31658 };
      municipio.fkIdUf = fkIdUf;

      const ufCollection: IUf[] = [{ idUf: 43119 }];
      jest.spyOn(ufService, 'query').mockReturnValue(of(new HttpResponse({ body: ufCollection })));
      const additionalUfs = [fkIdUf];
    ...
    it('Should update editForm', () => {
      ...
      expect(comp.ufsSharedCollection).toContain(fkIdUf);
    });
  });
  ...
  describe('save', () => {
    it('Should call update service on save for existing entity', () => {
      ...
      const municipio = { idMunicipio: 123 };
  ...
  describe('Tracking relationships identifiers', () => {
    describe('trackUfByIdUf', () => {
      it('Should return tracked Uf primary key', () => {
        const entity = { idUf: 123 };
        const trackResult = comp.trackUfByIdUf(0, entity);
        expect(trackResult).toEqual(entity.idUf);
      });
    });
  });
});
